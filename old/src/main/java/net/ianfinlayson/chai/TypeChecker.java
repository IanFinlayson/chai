...
public class TypeChecker extends ChaiParserBaseVisitor<Type> {
    ...
    public Type visitFunctiondef(ChaiParser.FunctiondefContext ctx) {
        // grab the name and return type, if there is one
        ...
        Function func = new Function(name, ctx.statements(), returnType);
        ...
        if (ctx.paramlist() != null) {
            ...
            for (ChaiParser.ParamContext param : params) {
                if (param instanceof ChaiParser.NamedParamContext) {
                    ...
                    func.registerParameter(np.IDNAME().getText(), visit(np.type()), false);
                } else {
                    ...
                    // put it in
                    func.registerParameter(pname, inferred, true);
                }
            }
        }
        ...
        for (int i = 0; i < func.getNumParams(); i++) {
            putVar(func.paramNames(i), func.paramTypes(i), false, ctx.getStart().getLine());
        }
    ...
    public Type visitReturnStatement(ChaiParser.ReturnStatementContext ctx) {
        ...
        if (ctx.expression() == null) {
            if (currentFunctions.peek().getReturnType() != null) {
        ...
        // if there is a return type, make sure it matches the function type
            ...
            if (currentFunctions.peek().getReturnType() == null || !givenValue.equals(currentFunctions.peek().getReturnType())) {
    ...
    public Type visitFunctioncall(ChaiParser.FunctioncallContext ctx) {
        ...
        return callee.typeCheck(args, ctx.getStart().getLine(), this);
    }
    ...
    // this recurses through the destructure expressions and does type
    // checking, and builds out new variables being introduced
    private void walkDestructures(ChaiParser.DestructureContext destr, ArrayList<String> newvars, Type expr) {
        ...
        if (destr instanceof ChaiParser.LiteralDestrContext) {
            // literal: just make sure type matches
            Type given = visit(((ChaiParser.LiteralDestrContext) destr).literal());
            ...
            putVar(name, expr, true, destr.getStart().getLine());
            newvars.add(name);
    ...
    public Type visitMatchStatment(ChaiParser.MatchStatmentContext ctx) {
        ...
        for (ChaiParser.CaselineContext caseline : ctx.caseline()) {
            ...
            // walk the destructures for this case line
            walkDestructures(destr, newvars, expr);
